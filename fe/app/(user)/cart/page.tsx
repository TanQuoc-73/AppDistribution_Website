...
export default function CartPage() {
  ...
  function handleCheckout() {
    ...
    createOrder({ productIds: items.map((i) => i.productId) });
  }
          ...
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 rounded-lg border border-neutral-800 bg-neutral-900 p-4">
              {item.product.thumbnailUrl && (
                <div className="relative h-16 w-24 overflow-hidden rounded bg-neutral-800">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-neutral-400">
                  {item.product.isFree ? 'Free' : `$${parseFloat(item.product.price).toFixed(2)}`}
                </p>
              </div>
              <button
